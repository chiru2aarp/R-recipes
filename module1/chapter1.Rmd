...
aggregate(x=iris[,1:4],by=list(iris$Species),FUN=mean)
 library(reshape)
 # melt from reshape : will perform the variable rearange based on the given feature.
 # by melting down to all variables associated to the given feature. 150 * 5 became 600(due to 150 * 4) * 3
...
```
Illustration of pch = 0:25 symbols to plot 21: 0-filled
