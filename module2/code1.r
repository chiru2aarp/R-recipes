...
plot(Income, Sales,col = "#4646ea", pch = 10) 
# using a different symbol. type ?points in r console window to learn more
plot(Income, Sales,col = rgb(red = 0.5,green =0.5,blue =1), pch = 10) 
# assigning rgb color type ?rgb in r console window to learn more
plot(Income, Sales,col =rainbow(3), pch = 20)
dev.off()
# using the rainbow to assign 3 different colors. type ?rainbow in r console window to learn more
# ******************************
# **recipe 2 :Abline and text***
# ******************************
#setwd("your path") 
# use setwd() to set your working directory. This code if used will give an error as it needs path
child = read.csv("~/../Downloads/Analytics/Books/R Recipes/module 2/1/data/chlmort.csv", 
                 header = TRUE, sep =",")
summary(child)
head(child)
 plot(child$gdp_bil, child$child, pch = 20, col = "#756bb1", xlim=c(0,max(child$gdp_bil)), 
      ylim = c(0,190), xlab = "GDP in Billions in current US$", 
      ylab ="Child Mortality rate", main = "child Mortality Rate in selected countries for 2012")
abline(h = (36.18), lwd = 1, col = "red")
text(8000,25,labels = c("Luxemborg"), cex = 3)
...
child = read.csv("~/../Downloads/Analytics/Books/R Recipes/module 2/1/data/chlmort.csv",
                 header = TRUE, sep =",")
summary(child)
#lpar : line paramerters and lwd : line width, lty: line type...
scatter.smooth(child$gdp_bil, child$child, pch = 20, lwd =0.75, col = "Blue", 
               lpars = list(lty = 2, col ="black", lwd = 2),
...
# **********************************************
income = read.csv("~/../Downloads/Analytics/Books/R Recipes/module 2/1/data/ginivsgdp1.csv", 
                  header = TRUE)
plot(income$gdp_ann,income$Gini,pch = 20, col = c(income$Presidents), type = "o",
     xlab =" GDP of USA", ylab = "Gini coefficient",main = "Inequality in USA", 
     xaxp = c(0,18000,10))
cols = as.numeric(income$Presidents)
legend("bottomright",fill = c(6,7,4,2,9,5,3,1,8), 
       legend = c("Johnson","Nixon","Ford","Carter","Reagan","G.Bush","Clinton","Bush","Obama"), bty = "n",cex = 0.7)
# **********************************************
...
plot(scaterp)
# add the trendline
scaterp1 = gvisScatterChart(scater, option= list(width = 500, height = 600,legend = "none",
            title = "Reltionship between Inequality and GDP growth in USA", 
            hAxis = "{title :'GDP'}", vAxis = "{title :'Gini'}", 
            trendlines="{0:{type : 'linear'}}"))
plot(scaterp1)
# add the equation and coefficients
scaterp2= gvisScatterChart(scater, option= list(width = 500, height = 600,legend = "none",
          title = "Reltionship between Inequality and GDP growth in USA", 
          hAxis = "{title :'GDP'}", vAxis = "{title :'Gini'}", 
          trendlines="{0:{type : 'linear', visibleInLegend: true}}"))
plot(scaterp2)
# add coefficient of Determination to the plot
scaterp3 = gvisScatterChart(scater, option= list(width = 650, height = 600,legend = "none",
            title = "Reltionship between Inequality and GDP growth in USA" ,
						hAxis = "{title :'GDP'}", vAxis = "{title :'Gini'}", 
						trendlines="{0:{type : 'linear', visibleInLegend: true, showR2: true}}"))
plot(scaterp3)
#adding data opacity
scaterp4 = gvisScatterChart(scater, option= list(width = 650, height = 600, legend = "none",
            title = "Reltionship between Inequality and GDP growth in USA",
						hAxis = "{title :'GDP'}",	vAxis = "{title :'Gini'}", dataOpacity = 0.8,
						trendlines="{0:{type : 'linear', visibleInLegend: true, showR2: true}}"))
plot(scaterp4)
# **********************************************
# **********Theres More*************************
...
#install.packages("googleVis")
#library(googleVis)
income = read.csv("~/../Downloads/Analytics/Books/R Recipes/module 2/1/data/ginivsgdp2.csv", 
                  header = TRUE)
scaterp = gvisScatterChart(income, option= list(width = 650, height = 600, dataOpacity = 0.8, 
          title = "Reltionship between Inequality and GDP growth in USA", hAxis = "{title :'GDP'}",
					vAxis = "{title :'Gini'}"))
plot(scaterp)
# **********************************************
# ***********Recipe 8 barplot*******************
...
data = read.csv("~/../Downloads/Analytics/Books/R Recipes/module 2/1/data/infant.csv", 
                header = TRUE)
...
abline(h = 12.2, lwd =2, col = "blue", lty =2)
# **********************************************
# ********recipe 9 - interactive barplots******
# **********************************************
#install.packages("googleVis")
#library(googleVis)
stock = read.csv("~/../Downloads/Analytics/Books/R Recipes/module 2/1/data/spq.csv", 
                 header = TRUE)
barpt = gvisBarChart(stock, xvar = "Date", yvar = c("Returns"),
       options = list(orientation = "horizontal", width = 1400, height = 500,
         ...
         legend = "none", hAxis = "{title :'Time Period',titleTextStyle :{color:'red'}}",
         vAxis = "{title : 'Returns(%)', ticks : [-12,-6,0,6, 12],titleTextStyle :{color: 'red'}}",
         bar = "{groupWidth: '100%'}"))
plot(barpt)
# **********************************************
...
frt = read.csv("~/../Downloads/Analytics/Books/R Recipes/module 2/1/data/fertility.csv", 
               header = TRUE, sep =",")
...
# **********************************************
base = read.csv("~/../Downloads/Analytics/Books/R Recipes/module 2/1/data/disable.csv")
...
baseball = gvisTimeline(data = data, rowlabel ="position",start ="start", end = "end",barlabel ="player" , 
                        option = list(width = 1000, height = 900,timeline="{singleColor :'#002A3E'}"))
plot(baseball)
# **********************************************
# ***Recipe 14 -Merging Histogram****
# **********************************************

stk = read.csv("~/../Downloads/Analytics/Books/R Recipes/module 2/1/data/stock_cor.csv", 
               header = TRUE, sep = ",")
...
# **********************************************
crm = read.csv("~/../Downloads/Analytics/Books/R Recipes/module 2/1/data/crimeusa.csv", 
               header = TRUE, sep =",")
bub1 = gvisBubbleChart(crm,idvar = "States",xvar= "Robbery", yvar= "Burglary", sizevar ="Population", colorvar = "Year",
			options = list(legend = "none",width = 900, height = 600,title =" Crime per State in 2012", 
			               sizeAxis ="{maxSize : 40, minSize :0.5}",vAxis = "{title : 'Burglary'}",hAxis= "{title : 'Robbery'}"))
plot(bub1)
bub2 = gvisBubbleChart(crm,idvar = "States",xvar= "Robbery", yvar= "Burglary",sizevar ="Population",
                       options = list(legend = "none",width = 900, height = 600,title =" Crime per State in 2012", 
                                      sizeAxis ="{maxSize : 40, minSize :0.5}",
                                      vAxis = "{title : 'Burglary'}",hAxis= "{title : 'Robbery'}"))
plot(bub2)
...
library(plotrix)
sales = read.csv("~/../Downloads/Analytics/Books/R Recipes/module 2/1/data/waterf.csv")
staircase.plot(sales$value, totals= sales$logic, labels = sales$labels, total.col = c("lightgreen"),
               inc.col = c("blue","red","red","blue","blue","blue","red","red","blue","blue","red","blue"),
               main ="Waterfall Plot showing financial data")
