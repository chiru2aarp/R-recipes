...
auto <- read.csv("~/Downloads/Books/Analytics_DS/R/R Recipes/module 1/11/data/auto-mpg.csv")
...
#2. Process rows and columns as a whole using the apply function
...
auto <- read.csv("~/Downloads/Books/Analytics_DS/R/R Recipes/module 1/11/data/auto-mpg.csv", stringsAsFactors=FALSE)

lapply(c(1,2,3), sqrt)

x <- list(a = 1:10, b = c(1,10,100,1000), c=seq(5,50,by=5))
lapply(x,mean)
class(lapply(x,mean)) # list
sapply(x,mean)
class(sapply(x,mean)) # mumeric
sapply(auto[,2:8], min)
...
sapply(auto[,2:6], min)
sapply(auto[,2], min) # doesnt work as expected... 
...
emp <- read.csv("~/Downloads/Books/Analytics_DS/R/R Recipes/module 1/11/data/employees.csv", stringsAsFactors=FALSE)
dept <- read.csv("~/Downloads/Books/Analytics_DS/R/R Recipes/module 1/11/data/departments-1.csv", stringsAsFactors=FALSE)
...
dept <- read.csv("~/Downloads/Books/Analytics_DS/R/R Recipes/module 1/11/data/departments-2.csv", stringsAsFactors=FALSE)
deptDT <- data.table(dept)
